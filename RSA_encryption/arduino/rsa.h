...
long* string2ascii_int_list(char* string) {
	...
	for (long i = 0; i < length; ++i) {
		ascii_int_list[i] = (long)(string[i]);
...
char* ascii_int_list2string(long* ascii_int_list, int length) {
	char* string;
	//int length = sizeof(ascii_int_list);
...
long* rsa_encrypt(char* plaintext, long* key) {
    ...
    Serial.write("chipertext_list["); Serial.print(i); Serial.print("] = "); Serial.println((ciphertext_list[i]));
  }
...
char* rsa_decrypt(long* ciphertext_list, long* key, int length) {
	//long n = key[0];
	//long d = key[1];
	long* plaintext_list;
	//int length = sizeof(ciphertext_list);
	plaintext_list = new long[length];
	for(int i = 0; i < length; ++i) {
	  plaintext_list[i] = raiseto_mod(ciphertext_list[i], key[1], key[0]);
        }    
	return ascii_int_list2string(plaintext_list, length);
}
